...
class APIActor extends Actor with ActorLogging {
  ...
  def generateTwitterDataFlow(preProcessingFlow: Flow[Status, Status, NotUsed]#Repr[String],
                              searchText: Option[String], geolocated: Boolean)
                             (implicit executionContext: ExecutionContext) = {
    val (source, closeFunction) = TwitterSource.createSource(searchText, geolocated)
  ...
  def requestHandler: HttpRequest => HttpResponse = {
    case req@HttpRequest(GET, Uri.Path("/tweets"), _, _, _) =>
      val searchText = req.uri.query().get("search")
          ...
          req.uri.query().get("geolocated") match {
            case Some("true") =>
              val processFlow = FlowHelper
                ...
                .map(TweetWithGeoLocation.format.writes(_).toString())
              upgrade
                .handleMessages(generateTwitterDataFlow(processFlow, searchText, geolocated = true))
            case _ =>
              val processFlow = FlowHelper
                .getTweetsWordCountProcessingPipeline
                .map(TweetWordCount.format.writes(_).toString())
              upgrade
                .handleMessages(generateTwitterDataFlow(processFlow, searchText, geolocated = false))
          }
